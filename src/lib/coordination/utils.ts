import type { ProposalOrderingCandidate } from './types'

export function deriveCoordinationId(intentIds: string[]): string {
  const normalized = [...intentIds].sort().join(',')
  return sha256Hex(new TextEncoder().encode(normalized))
}

export function compareProposals(
  a: ProposalOrderingCandidate,
  b: ProposalOrderingCandidate,
): number {
...
function sha256Hex(input: Uint8Array): string {
  ...
  words[originalBitLength >> 5] |= 0x80 << (24 - (originalBitLength % 32))
  words[(((originalBitLength + 64) >> 9) << 4) + 15] = originalBitLength
  ...
  for (let offset = 0; offset < words.length; offset += 16) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const ch = (e & f) ^ (~e & g)
      const temp1 = (((((h + s1) | 0) + ch) | 0) + ((K[index]! + w[index]!) | 0)) | 0
...
function bytesToWords(input: Uint8Array): number[] {
  ...
  for (let index = 0; index < input.length; index += 1) {
    words[index >> 2] ??= 0
    words[index >> 2] |= input[index]! << (24 - (index % 4) * 8)
  }
